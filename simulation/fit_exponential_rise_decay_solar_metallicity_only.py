...
filename = os.path.join(
    ...
    "spectrum_1.0_gyr_2.0_rise_decay_1.0_zsolar.npy",
)
...
for metallicity in metallicity_list:
    for age in [0.5, 1.0, 2.0, 4.0, 5.0, 6.0, 7.0, 11.0]:
        filename = os.path.join(
            "output",
            "exponential_rise_decay",
            f"spectrum_{age:.1f}_gyr_{tau:.1f}_rise_decay_{metallicity:.1f}_zsolar.npy",
        )
        ...
        noise = np.ones_like(galaxy_log_rebinned) * 1e-6
        ...
        plt.savefig(
            os.path.join(
                "output",
                "exponential_rise_decay",
                f"spectrum_{age:.1f}_gyr_{tau:.1f}_rise_decay_{metallicity:.1f}_zsolar_fitted.png",
            )
        )
