...
if os.path.exists("sfh_voronoi.npy"):
    ...
    for i, (sfh, spatial, stellar_mass, remnant_mass) in enumerate(
        zip(
            firefly_sfh_list,
        ...
        for voronoi_id in voronoi_id_list:
            ...
            mass_voronoi_i = (
                10.0 ** stellar_mass[arg][2] + 10.0 ** remnant_mass[arg][2]
            )
            sfh_voronoi_i = np.array(sfh[arg][:, 2])
            sfh_mask = sfh_voronoi_i > 0.0
            age = np.around(
                np.array(10.0 ** sfh[arg][:, 0][sfh_mask] * 1e9), decimals=-5
            )
            sfh_voronoi_i_vid = np.zeros_like(input_age)
            for j, a in enumerate(age):
                idx = np.where(input_age == a)[0][0]
                sfh_voronoi_i_vid[idx] = (
                    sfh_voronoi_i[sfh_mask][j] * mass_voronoi_i
                )
