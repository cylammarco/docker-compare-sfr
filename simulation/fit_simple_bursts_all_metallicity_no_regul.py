...
for metallicity in np.arange(-1.0, 1.1, 0.25):
    for age in np.arange(0.5, 13.5, 0.5):
        ...
        ax4.plot(
            age_grid, sfr / np.nanmax(sfr), label="Recovered (light-weighted)"
        )
        ax4.plot(
            age_grid,
        ...
        plt.savefig(
            os.path.join(
                "output",
                "simple_bursts",
                "spectrum_{}_gyr_{}_logzsol_fitted_with_all_metallicity_no_regul.png".format(
                    age, metallicity
                ),
            )
        )
