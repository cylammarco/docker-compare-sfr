...
def find_reg(
    ...
    chi2_diff = abs(pp_temp.chi2 - desired_chi2)
    print("")
    print(
        "================================================================================"
    )
    print("Current regularisation value is: {}.".format(reg_guess))
    print("Current Chi^2: {}".format(pp_temp.chi2))
...
def get_uncertainty(
    ...
    for _ in range(100):
        ...
        light_weights = pp_temp.weights  # Exclude gas templates weights
        ...
        galaxy_output.append(pp_temp.bestfit)
...
filename = os.path.join(
    ...
    "spectrum_1.0_gyr_1.0_zsolar_with_noise_10.npy",
)
    ...
    for sn in ["10", "20", "50"]:
        for age in [0.5, 1.0, 2.0, 4.0, 5.0, 6.0, 7.0, 11.0]:
            filename = os.path.join(
                "output",
                "simple_bursts_with_noise",
                f"spectrum_{age:.1f}_gyr_1.0_zsolar_with_noise_{sn}.npy",
            )
            ...
            plt.savefig(
                os.path.join(
                    "output",
                    "simple_bursts_with_noise",
                    f"spectrum_{age:.1f}_gyr_{metallicity:.1f}_zsolar_with_noise_fitted_{sn}.png",
                )
            )
