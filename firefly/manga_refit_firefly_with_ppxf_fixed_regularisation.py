...
wave_new = np.arange(3650.0, 8500.0, FWHM_gal)
...
# Loop through all the spexels here
for _id in vorid_list:
    ...
    # IVAR = Inverse mean-square error
    galaxy_err = 1.0 / data["IVAR"].data[:, x, y]
    galaxy_err = np.sqrt(galaxy_err)
    snr = np.nansum(galaxy) / np.nansum(galaxy_err)
    ...
    plt.savefig(
        "manga-10001/manga-10001-6104-{}-averaged-regul-{}.png".format(
            _id, float(snr)
        )
    )
