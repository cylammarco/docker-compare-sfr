...
def mass_to_light(weights, band="r", quiet=False):
    ...
    """
    assert miles.age_grid.shape == miles.metal_grid.shape ==\
    ...
    for j in range(miles.n_ages):
        for k in range(miles.n_metal):
            p1 = (np.abs(miles.age_grid[j, k] - Age1) <
                  0.001) & (np.abs(miles.metal_grid[j, k] - MH1) <
                            0.01) & (np.abs(1.30 - slope1) < 0.01)
            mass_no_gas_grid[j, k] = m_no_gas[p1]

            p2 = (np.abs(miles.age_grid[j, k] - _Age2) <
                  0.001) & (np.abs(miles.metal_grid[j, k] - _MH2) <
                            0.01) & (np.abs(1.30 - _slope2) < 0.01)
...
def find_ref(reg_guess, templates, galaxy, noise, velscale, start, goodpixels,
             plot, moments, degree, mdegree, clean, vsyst, ftol, fixed, lam,
             linear_method, reg_dim, component):
...
if not os.path.exists('..' + os.sep + 'synthetic_spectra' + os.sep +
                      'fitted_model'):
...
sigmaV = np.linspace(0.5, 4,
                     m)  # Range of sigma in *pixels* [=sigma(km/s)/velScale]

degree = 4
mdegree = 0
fixed = [0, 0, 0, 0]

for sf_type in ['sb00', 'ed05']:

    for z in [-0.71, -0.40, 0.0, 0.22]:

        for t in 10.**np.arange(-1.0, 1.3, 0.1):

            # Star burst
            wave, galaxy = np.load(
                '..' + os.sep + 'synthetic_spectra' + os.sep +
                'sp_{0}_z{1:1.2f}_t{2:2.2f}.npy'.format(sf_type, z, t)).T
            ...
            for j, (vel, sigma) in enumerate(zip(velV, sigmaV)):
                ...
                galaxy_rebinned = signal.fftconvolve(
                    galaxy_log_rebinned, losvd,
                    mode="same")  # Convolve the oversampled spectrum
                galaxy_rebinned = rebin(
                    galaxy_rebinned,
                    factor)  # Integrate spectrum into original spectral pixels
                ...
                templates = miles.templates.reshape(miles.templates.shape[0],
                                                    -1)

                noise = galaxy_rebinned / sn  # 1sigma error spectrum
                galaxy_rebinned = np.random.normal(
                    galaxy_rebinned, noise)  # Add noise to the galaxy spectrum
                ...
                _weights = pp.weights
                weights.append(_weights.reshape(reg_dim) /
                               _weights.sum())  # Normalized
            ...
            if (sf_type == 'sb00'):
                ...
                sfr_input[np.argmin(np.abs(age - t))] = 1.
            else:
            ...
            ax1.plot(np.exp(wave_rebinned),
                     pp.bestfit,
                     color='black',
                     label='Fitted')
            ax1.scatter(np.exp(wave_rebinned),
                        galaxy_rebinned - pp.bestfit,
                        color='green',
                        s=2,
                        label='Residual')
            ax1.grid()
            ax1.set_xlim(min(np.exp(wave_rebinned)),
                         max(np.exp(wave_rebinned)))
            ax1.set_xlabel('Wavelength / A')
            ax1.set_ylabel('Relative Flux')
            ax1.legend()

            ax3.imshow(input_weights,
                       origin='lower',
                       extent=[
                           min(age),
                           max(age),
            ...
            plt.savefig('..' + os.sep + 'synthetic_spectra' + os.sep +
                        'figure' + os.sep +
                        'sp_{0}_z{1:1.2f}_t{2:2.2f}.png'.format(sf_type, z, t))
            np.save(
                '..' + os.sep + 'synthetic_spectra' + os.sep + 'sfr' + os.sep +
                'sp_{0}_z{1:1.2f}_t{2:2.2f}_sfr'.format(sf_type, z, t),
                np.column_stack((age, sfr)))
            np.save(
                '..' + os.sep + 'synthetic_spectra' + os.sep + 'fitted_model' +
                os.sep + ''
                'sp_{0}_z{1:1.2f}_t{2:2.2f}_fitted_model_pp'.format(
                    sf_type, z, t), pp)
            np.save(
                '..' + os.sep + 'synthetic_spectra' + os.sep + 'fitted_model' +
                os.sep + ''
                'sp_{0}_z{1:1.2f}_t{2:2.2f}_fitted_model_weight'.format(
                    sf_type, z, t), weights)
