...
if not os.path.exist('synthetic_spectra'):
    os.path.mkdir('synthetic_spectra')
...
for age in np.arange(1, 15):
    ...
    plt.plot(wave,
             spec,
             label=r'Star Burst 0.5 Gyr, log(Z/Z$_\odot$) = 0.0, age = {} Gyr'.
             format(age))
    np.save('synthetic_spectra/sp_sb05_z00_t{}'.format(age),
            np.column_stack((wave, spec)))
    wave, spec = sp_sb05_zp01.get_spectrum(tage=age)
    plt.plot(wave,
             spec,
             label=r'Star Burst 0.5 Gyr, log(Z/Z$_\odot$) = 0.1, age = {} Gyr'.
             format(age))
    np.save('synthetic_spectra/sp_sb05_zp01_t{}'.format(age),
            np.column_stack((wave, spec)))
    ...
    plt.plot(
        ...
        label=r'Star Burst 0.5 Gyr, log(Z/Z$_\odot$) = +0.1, age = {} Gyr'.
        format(age))
    np.save('synthetic_spectra/sp_sb05_zm01_t{}'.format(age),
            np.column_stack((wave, spec)))
    wave, spec = sp_sb05_zp02.get_spectrum(tage=age)
    plt.plot(wave,
             spec,
             label=r'Star Burst 0.5 Gyr, log(Z/Z$_\odot$) = 0.2, age = {} Gyr'.
             format(age))
    np.save('synthetic_spectra/sp_sb05_zp02_t{}'.format(age),
            np.column_stack((wave, spec)))
    ...
    plt.plot(
        ...
        label=r'Star Burst 0.5 Gyr, log(Z/Z$_\odot$) = -0.2, age = {} Gyr'.
        format(age))
    np.save('synthetic_spectra/sp_sb05_zm02_t{}'.format(age),
            np.column_stack((wave, spec)))
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('synthetic_spectra/sb05_t{}'.format(age))
    # Exponential
    plt.figure(2)
    plt.clf()
    ...
    plt.plot(
        wave,
        spec,
        label=r'$\tau$ = 1 Gyr, log(Z/Z$_\odot$) = 0.0, age = {} Gyr'.format(
            age))
    np.save('synthetic_spectra/sp_ed1_z00_t{}'.format(age),
            np.column_stack((wave, spec)))
    wave, spec = sp_ed1_zp01.get_spectrum(tage=age)
    plt.plot(
        wave,
        spec,
        label=r'$\tau$ = 1 Gyr, log(Z/Z$_\odot$) = 0.1, age = {} Gyr'.format(
            age))
    np.save('synthetic_spectra/sp_ed1_zp01_t{}'.format(age),
            np.column_stack((wave, spec)))
    ...
    plt.plot(
        ...
        label=r'$\tau$ = 1 Gyr, log(Z/Z$_\odot$) = -0.1, age = {} Gyr'.format(
            age))
    np.save('synthetic_spectra/sp_ed1_zm01_t{}'.format(age),
            np.column_stack((wave, spec)))
    wave, spec = sp_ed1_zp02.get_spectrum(tage=age)
    plt.plot(
        wave,
        spec,
        label=r'$\tau$ = 1 Gyr, log(Z/Z$_\odot$) = 0.2, age = {} Gyr'.format(
            age))
    np.save('synthetic_spectra/sp_ed1_zp02_t{}'.format(age),
            np.column_stack((wave, spec)))
    ...
    plt.plot(
        ...
        label=r'$\tau$ = 1 Gyr, log(Z/Z$_\odot$) = -0.2, age = {} Gyr'.format(
            age))
    np.save('synthetic_spectra/sp_ed1_zm02_t{}'.format(age),
            np.column_stack((wave, spec)))
    plt.xlabel('Wavelength (A)')
    plt.ylabel('Flux (erg / s / cm / cm / A)')
    plt.xlim(3500, 8500)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('synthetic_spectra/ed1_t{}'.format(age))
'''
...
for age in np.arange(1, 15):
    ...
    plt.figure(1)
    plt.clf()
    wave, spec = np.load('synthetic_spectra/sp_sb05_z00_t{}.npy'.format(age)).T
    plt.plot(wave, spec, label=r'Star Burst 0.5 Gyr, log(Z/Z$_\odot$) = 0.0, age = {} Gyr'.format(age))
    np.save('synthetic_spectra/sp_sb05_z00_t{}'.format(age),
            np.column_stack((wave, spec)))
    wave, spec = np.load('synthetic_spectra/sp_sb05_zp01_t{}.npy'.format(age)).T
    plt.plot(wave, spec, label=r'Star Burst 0.5 Gyr, log(Z/Z$_\odot$) = 0.1, age = {} Gyr'.format(age))
    np.save('synthetic_spectra/sp_sb05_zp01_t{}'.format(age),
            np.column_stack((wave, spec)))
    wave, spec = np.load('synthetic_spectra/sp_sb05_zm01_t{}.npy'.format(age)).T
    plt.plot(wave, spec, label=r'Star Burst 0.5 Gyr, log(Z/Z$_\odot$) = +0.1, age = {} Gyr'.format(age))
    np.save('synthetic_spectra/sp_sb05_zm01_t{}'.format(age),
            np.column_stack((wave, spec)))
    wave, spec = np.load('synthetic_spectra/sp_sb05_zp02_t{}.npy'.format(age)).T
    plt.plot(wave, spec, label=r'Star Burst 0.5 Gyr, log(Z/Z$_\odot$) = 0.2, age = {} Gyr'.format(age))
    np.save('synthetic_spectra/sp_sb05_zp02_t{}'.format(age),
            np.column_stack((wave, spec)))
    wave, spec = np.load('synthetic_spectra/sp_sb05_zm02_t{}.npy'.format(age)).T
    plt.plot(wave, spec, label=r'Star Burst 0.5 Gyr, log(Z/Z$_\odot$) = -0.2, age = {} Gyr'.format(age))
    np.save('synthetic_spectra/sp_sb05_zm02_t{}'.format(age),
            np.column_stack((wave, spec)))
    plt.xlabel('Wavelength (A)')
    ...
    plt.figure(2)
    plt.clf()
    wave, spec = np.load('synthetic_spectra/sp_ed1_z00_t{}.npy'.format(age)).T
    plt.plot(wave, spec, label=r'$\tau$ = 1 Gyr, log(Z/Z$_\odot$) = 0.0, age = {} Gyr'.format(age))
    np.save('synthetic_spectra/sp_ed1_z00_t{}'.format(age),
            np.column_stack((wave, spec)))
    wave, spec = np.load('synthetic_spectra/sp_ed1_zp01_t{}.npy'.format(age)).T
    plt.plot(wave, spec, label=r'$\tau$ = 1 Gyr, log(Z/Z$_\odot$) = 0.1, age = {} Gyr'.format(age))
    np.save('synthetic_spectra/sp_ed1_zp01_t{}'.format(age),
            np.column_stack((wave, spec)))
    wave, spec = np.load('synthetic_spectra/sp_ed1_zm01_t{}.npy'.format(age)).T
    plt.plot(wave, spec, label=r'$\tau$ = 1 Gyr, log(Z/Z$_\odot$) = -0.1, age = {} Gyr'.format(age))
    np.save('synthetic_spectra/sp_ed1_zm01_t{}'.format(age),
            np.column_stack((wave, spec)))
    wave, spec = np.load('synthetic_spectra/sp_ed1_zp02_t{}.npy'.format(age)).T
    plt.plot(wave, spec, label=r'$\tau$ = 1 Gyr, log(Z/Z$_\odot$) = 0.2, age = {} Gyr'.format(age))
    np.save('synthetic_spectra/sp_ed1_zp02_t{}'.format(age),
            np.column_stack((wave, spec)))
    wave, spec = np.load('synthetic_spectra/sp_ed1_zm02_t{}.npy'.format(age)).T
    plt.plot(wave, spec, label=r'$\tau$ = 1 Gyr, log(Z/Z$_\odot$) = -0.2, age = {} Gyr'.format(age))
    np.save('synthetic_spectra/sp_ed1_zm02_t{}'.format(age),
            np.column_stack((wave, spec)))
    plt.xlabel('Wavelength (A)')
