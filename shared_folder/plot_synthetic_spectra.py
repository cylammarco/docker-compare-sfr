...
for i, f in enumerate([miles_filelist[0]]):
    ...
    if metallicity[0] == 'p':
        ...
        print('This should not happen. Check the filename: {}.'.format(filename))
    age = float(filename.split('T')[-1].split('_iPp')[0])
    miles_data = fits.open(f)[0]
    fsps_miles_data = fits.open(fsps_miles_filelist[i])[0]
    ...
    fsps_flux_model = fsps_miles_data.data
    ...
    figure(1)
    clf()
    plot(wave_model, flux_fsps, label='FSPS generated')
    plot(wave_model, fsps_flux_model, label='FSPS template')
