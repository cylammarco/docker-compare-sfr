...
from simulation.dtd_functions import get_dtd, get_tophat_dtd
...
def likelihood_voronoi(
    ...
    if (dtd_guess < -30).any() or (dtd_guess > -8).any():
        return np.inf
...
age_bin_edge = (age_bin[1:] + age_bin[:-1]) / 2.0
...
dtd_maoz_2012 = [140e-14, 25.1e-14, 1.83e-14]
dtd_maoz_2012_err = [30e-14, 6.3e-14, 0.42e-14]
time_maoz_2012 = [0.21e9, 1.41e9, 8.2e9]
time_maoz_2012_err = [0.21e9, 0.99e9, 5.8e9]
...
sn_list = np.load("sn_list_firefly_asassn.npy")
sn_mask = sn_list > 0

# time of observation (2015 - 2021)
t_obs = 7.0
# discovery efficiency
epsilon = 0.5
# a flat input DTD function
dtd_bin = np.ones_like(age_bin) * -10
...
np.save(
    "recovered_dtd_rate_firefly_asassn_binned",
    answer,
)
np.save(
    "recovered_dtd_rate_firefly_asassn_binned_zero_inflated_lambert",
...
alpha = np.zeros((len(age_bin), len(age_bin)))
...
plt.errorbar(
    time_maoz_2012,
    dtd_maoz_2012,
    yerr=dtd_maoz_2012_err,
    xerr=time_maoz_2012_err,
    label="MMB 2012",
    fmt=".",
    color='black'
)
...
plt.plot(input_age, dtd_itp(input_age), label=r"Expected DTD ($\beta=-1.12$)")
...
plt.tight_layout()
plt.savefig("best_fit_dtd_rate_firefly_asassn_binned.png")
...
plt.plot(
    age_bin, np.sum(sfh_voronoi_binned, axis=0), label="total"
)
plt.xlabel("Time (yr)")
...
plt.tight_layout()
plt.grid()
plt.savefig("sfh_with_sn_asassn_binned.png")
