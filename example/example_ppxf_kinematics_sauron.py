...
#   V2.0.1: Included instructions for high-redshift usage. Thanks to Paul
#       Westoby for useful feedback on this issue. MC, Oxford, 27 November 2008
...
def ppxf_example_kinematics_sauron():
    ...
    # lamRange1 = lamRange1/(1+z) # Compute approximate restframe wavelength
    # range FWHM_gal = FWHM_gal/(1+z)   # Adjust resolution in Angstrom
    ...
    FWHM_tem = 2.51
    # adopts 2x higher spectral sampling for templates than for galaxy
    velscale_ratio = 2

    # Extract the wavelength range and logarithmically rebin one spectrum
    # to a velocity scale 2x smaller than the SAURON galaxy spectrum, to
    ...
    # wavelength. For this reason an extra velocity shift DV has to be applied
    # to the template to fit the galaxy spectrum. We remove this artificial
    # shift by using the keyword VSYST in the call to PPXF below, so that all
    # velocities are measured with respect to DV. This assume the redshift is
    # negligible. In the case of a high-redshift galaxy one should de-redshift
    # its wavelength to the rest frame before using the line below (see above).
    #
    c = 299792.458
...
if __name__ == '__main__':
    ...
    plt.savefig('kinematics_sauron_fit.jpg')
